    ...
    if(v <= numVertices){
        for(int i = 1; i<= numVertices; ++i){
            if(adjMatrix[v][i] == 1 || adjMatrix[i][v] == 1){
                check = true;
                cout << "\nFirst Remvoe Edge Between vertex " << "\033[1;31m"<<lettring[v-1]<<"\033[0m" << " and " << "\033[1;31m"<<lettring[i-1]<<"\033[0m" << ", Isolate Vertex "<<"\033[1;32m"<<lettring[v-1] << "\033[0m" << " To Remove "<<endl;
            }
        }
    }
